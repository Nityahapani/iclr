...
def fisher_curvature_along_v(model, obj_ids: torch.Tensor, ctx_ids: torch.Tensor, v: torch.Tensor) -> float:
    """
    For each x=(obj,ctx) in the batch, compute grad_h log p(y_pred|x) . v.
    """
